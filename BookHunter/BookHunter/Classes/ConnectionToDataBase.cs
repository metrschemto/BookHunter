...
namespace BookHunter.Classes
{
    class ConnectionToDataBase
    {
        ...
        public static List<Classes.OrderItem> OrderItems (string OrderID)
        {
            ...
            MySqlConnection connection = ConnectionToDataBase.Connection();
            connection.Open();
            string Query = "select order_code, order_content.book_code, books.book_name, author_surname,author_patronymic,order_content.count,book_image_path,book_cost from authors,books,order_content where books.book_code = order_content.book_code and book_author = author_code and order_code = " + OrderID;
            MySqlCommand command = new MySqlCommand(Query, connection);
            MySqlDataReader reader = command.ExecuteReader();
            while (reader.Read())
            {
